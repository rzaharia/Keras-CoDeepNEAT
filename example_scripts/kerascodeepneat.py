...
def upload_blob(source_file_name):
    ...
    bucket = storage_client.bucket('p3-2020-bucket')
    blob = bucket.blob('output/' + os.path.basename(source_file_name))

    blob.upload_from_filename(source_file_name)

    print("File {} uploaded to {}.".format(source_file_name, 'output'))
...
class Population:
    ...
    def iterate_fitness_distributed_smarter(self, sc, data_rdd, training_epochs=1, validation_split=0.15, current_generation=0):
        ...
        for id, individual in enumerate(self.individuals):
            individual.generate(generation=current_generation)
            id_2_individual[id] = individual
            id_2_model.append((id, individual.model))
        ...
        SAMPLE_SIZE, TEST_SAMPLE_SIZE = self.datasets.SAMPLE_SIZE, self.datasets.TEST_SAMPLE_SIZE
        individuals_2_scores = models_rdd.map(lambda model_id_2_model: map(data_rdd.value,
                                                                           model_id_2_model[0], 
                                                                           model_id_2_model[1], 
                                                                           training_epochs, 
                                                                           validation_split, 
    ...
    def iterate_generations(self, generations=1, training_epochs=1, validation_split=0.15, mutation_rate=0.5, crossover_rate=0.2, elitism_rate=0.1, possible_components=None, possible_complementary_components=None, distributed=False, sc=None):
        ...
        data_x = self.datasets.training[0]
        data_y = self.datasets.training[1]
        test_data_x = self.datasets.test[0]
        test_data_y = self.datasets.test[1]
        data_rdd = sc.broadcast([data_x, data_y, test_data_x, test_data_y])
        ...
        for generation in range(generations):
            ...
            if distributed:
                ...
                iteration = self.iterate_fitness_distributed_smarter(sc, data_rdd, training_epochs, validation_split, current_generation=generation)
            else:
            ...
            try:
                ...
                upload_blob(f"{basepath}/models/best_generation_{generation}.h5")
            except:
    ...
    def train_full_model(self, individual, training_epochs, validation_split, custom_fit_args=None):
        ...
        upload_blob(f"{basepath}/models/full_model_indiv{individual.name}_blueprint{individual.blueprint.mark}.h5")

        with open(f'{basepath}training.json', 'w', encoding='utf-8') as f:
            json.dump(history.history, f, ensure_ascii=False, indent=4)

        upload_blob(f'{basepath}training.json')
        ...
        # upload_blob(f'{basepath}/images/history_acc')
        plt.clf()
        ...
        # upload_blob(f'{basepath}/images/history_loss')
